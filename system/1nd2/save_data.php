...
<?php
  ...
  $aData = json_decode(file_get_contents('a_data.json'), true);
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["team"]) && isset($_POST["betAmount"])) {
    ...
    if ($selectedTeam == "A") {
        ...
        // 기존 데이터 로드
        $existingData = json_decode(file_get_contents('a_data.json'), true);
        ...
        // 데이터 저장
        file_put_contents('a_data.json', json_encode($existingData));
        ...
        // 기존 데이터 로드
        $existingData = json_decode(file_get_contents('b_data.json'), true);
        if ($existingData === null) {
            $existingData = [];
        }
        ...
        // 데이터 저장
        file_put_contents('b_data.json', json_encode($existingData));
    }
