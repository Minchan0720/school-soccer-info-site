...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
   ...
   if (isset($_POST["team"]) && isset($_POST["betAmount"])) {
       ...
       if ($selectedTeam == "A") {
           ...
           file_put_contents('a_2_data.json', json_encode($data_a));
           ...
           file_put_contents('b_2_data.json', json_encode($data_b));
       }
   }
}
?>
...
// "2st_1" 및 "2st_2" 키의 존재 여부를 확인하고 해당 값을 설정
$second_team_1 = isset($json_a['2st']['2st_1']) ? $json_a['2st']['2st_1'] : "값이 없음";
$second_team_2 = isset($json_a['2st']['2st_2']) ? $json_a['2st']['2st_2'] : "값이 없음";
  ?>
...
if (isset($data['3st']['3st_1_score'])) {
    $first_score_1 = $data['3st']['3st_1_score'];
} else {
    $first_score_1 = "값이 없음";
}

if (isset($data['3st']['3st_2_score'])) {
    $team_members = $data['3st']['3st_2_score'];
} else {
    $team_members = "값이 없음";
}
...
if (isset($data['4st']['4st_1'])) {
    $first_score_2 = $data['4st']['4st_1'];
} else {
    $first_score_2 = "값이 없음";
}

if (isset($data['4st']['4st_2'])) {
    $team_members2 = $data['4st']['4st_2'];
} else {
    $team_members2 = "값이 없음";
}
...
<?php
// a_data.json 파일 로드
$data = json_decode(file_get_contents('a_2_data.json'), true);

// 중복을 제외한 유니크한 사용자 수 계산
$uniqueUsers = [];
if (is_array($data)) { // 데이터가 유효한 배열인지 확인
...
if ($userCount === null) { // null인 경우 0으로 대체
    $userCount = 0;
}
?>
<?php
// b_data.json 파일 로드
$data = json_decode(file_get_contents('b_2_data.json'), true);
...
if ($userCount2 === null) { // null인 경우 0으로 대체
    $userCount2 = 0;
}
?>
<!DOCTYPE html>
...
<form method="post" action="save_data2.php"> <!-- 폼 액션 변경 -->
<h1 id="title-text" style="text-align:left;">Noisy IT Webb23</h1> <!-- 제목 박스 -->
  <div class="team-box" data-team="A">
      <h2 class="title"><?php echo $second_team_1; ?></h2> <!-- JSON 데이터 사용 -->
  ...
  <div class="team-box" data-team="B">
      <h2 class="title"><?php echo $second_team_2; ?></h2> <!-- JSON 데이터 사용 -->
